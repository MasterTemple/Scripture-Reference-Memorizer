...
import { history, typedLetters, typedWords } from "./stores.js";
...
export function addTypedLetter(word, guess, isCorrect) {
	typedLetters.update((old) => [...old, { isCorrect, guess, word }]);
}
...
export function removeTypedLetter() {
	typedLetters.update((old) => old.slice(0, old.length - 1));
}
...
export function clearTypedLetters() {
	typedLetters.set([]);
}
